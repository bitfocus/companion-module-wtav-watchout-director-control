...
module.exports = function (self) {
  ...
  const timelineField = (id = 'timeline', label = 'Timeline') => ({
    type: 'dropdown', id, label, default: (tls[0] && tls[0].id) || '0',
  ...
  self.setActionDefinitions({
    timeline_transport: {
      ...
      options: [
        timelineField(),
        { type: 'dropdown', id: 'verb', label: 'Action', default: 'play', choices: [
          { id: 'play', label: 'Play' }, { id: 'pause', label: 'Pause' },
          { id: 'stop', label: 'Stop' }, { id: 'toggle', label: 'Toggle play/pause' }
        ] }
      ],
      callback: async (e) => {
        const id = await self.parseVariablesInString(String(e.options.timeline));
    ...
    timeline_play_at: {
      ...
      options: [
        timelineField(),
        { type: 'textinput', id: 'time', label: 'Start position (ms)', default: '0', useVariables: true }
      ],
      callback: async (e) => {
        const id = await self.parseVariablesInString(String(e.options.timeline));
        const time = Number(await self.parseVariablesInString(String(e.options.time)));
    ...
    timeline_jump: {
      ...
      options: [
        timelineField(),
        { type: 'dropdown', id: 'mode', label: 'Jump to', default: 'time', choices: [
          { id: 'time', label: 'Time (ms)' }, { id: 'cue', label: 'Cue id' } ] },
        { type: 'textinput', id: 'time', label: 'Time (ms)', default: '0', isVisible: (o) => o.mode === 'time', useVariables: true },
        { type: 'textinput', id: 'cue', label: 'Cue id', default: '', isVisible: (o) => o.mode === 'cue', useVariables: true },
        { type: 'dropdown', id: 'state', label: 'Then', default: 'play', choices: [
          { id: 'play', label: 'Play' }, { id: 'pause', label: 'Pause (hold)' } ] }
      ],
      callback: async (e) => {
        const id = await self.parseVariablesInString(String(e.options.timeline));
        const body = { state: e.options.state };
        if (e.options.mode === 'cue') body.cue = await self.parseVariablesInString(String(e.options.cue));
        else body.time = Number(await self.parseVariablesInString(String(e.options.time)));
    ...
    cueset_preset: {
      ...
      callback: async (e) => {
        ...
        if (!pair) { self.log('warn', 'cue set: pick a preset'); return; }
    ...
    set_variable: {
      ...
      options: [
        { type: 'dropdown', id: 'variable', label: 'Variable (by key)',
          ...
          allowCustom: true },
        { type: 'textinput', id: 'value', label: 'Value (in the variable’s own units)', default: '0', useVariables: true }
      ],
      callback: async (e) => {
        const key = await self.parseVariablesInString(String(e.options.variable));
        const value = Number(await self.parseVariablesInString(String(e.options.value)));
        if (!key) { self.log('warn', 'variable: an empty key would hit every variable — pick one'); return; }
        if (!isFinite(value)) { self.log('warn', 'variable: value is not a number'); return; }
