...
function encodePng(width, height, rgba) {
	...
	ihdr[10] = 0
	ihdr[11] = 0
	ihdr[12] = 0 // deflate / no filter / no interlace
	const stride = width * 4
	const raw = Buffer.alloc((stride + 1) * height)
	for (let y = 0; y < height; y++) {
		raw[y * (stride + 1)] = 0 // filter byte per scanline
...
// Render a horizontal fill bar as a base64 PNG. opts: { w,h, fg, bg, border, vertical }
// fraction is clamped to 0..1; a null/NaN fraction renders an empty bar.
export function barPng(fraction, opts) {
	opts = opts || {}
	const w = opts.w || 72,
		h = opts.h || 72
...
// A ready-to-use data URL for a Companion advanced-feedback png64 field.
export function barPngDataUrl(fraction, opts) {
	return 'data:image/png;base64,' + barPng(fraction, opts)
}
