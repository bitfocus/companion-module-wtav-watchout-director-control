...
// carry the small formatting/normalisation maths.
...
function packPair(groupId, presetId) { return JSON.stringify([String(groupId), String(presetId)]); }
function unpackPair(id) {
  try { const a = JSON.parse(id); if (Array.isArray(a) && a.length === 2) return { groupId: String(a[0]), presetId: String(a[1]) }; } catch (e) {}
  return null;
}

function timelineChoices(show) {
...
function variableChoices(show) {
  const vars = (show && show.variables) || [];
  return vars.filter(v => v.key).map(v => ({
    id: String(v.key),
    label: (v.name || v.key) + '  (' + fmtNum(v.min) + '…' + fmtNum(v.max) + ')'
  }));
}

function cuesetPresetChoices(show) {
  const groups = (show && show.cueSets) || [];
  const out = [];
  for (const g of groups) for (const p of (g.presets || [])) {
...
module.exports = {
  sanitizeId, packPair, unpackPair,
  timelineChoices, variableChoices, cuesetPresetChoices,
  fmtNum, fmtTime, fraction
};
