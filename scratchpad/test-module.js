...
const { apiUrl } = require('../src/api');
const { barPng, encodePng } = require('../src/bar');
...
console.log('util');
const show = {
  timelines: [{ id: '0', name: 'Main', folder: null }, { id: '1', name: 'Intro', folder: 'Openers/Sub' }],
  cueSets: [{ id: 'g1', name: 'Set01', presets: [{ id: 'a', name: 'Apple' }, { id: 'd', name: 'Deliver' }] }],
  variables: [{ key: '', name: 'masterVolume', min: 0, max: 100 }, { key: 'v1', name: 'Variable1', min: 0, max: 1 }]
};
...
const empty = barPng(0, { w: 40, h: 20 }), half = barPng(0.5, { w: 40, h: 20 }), full = barPng(1, { w: 40, h: 20 });
...
ok(barPng(2, { w: 20, h: 20 }) === full.length ? true : barPng(2, { w: 40, h: 20 }) === full, 'over-1 fraction clamps to full');
// IHDR of a 10x4 image encodes its dimensions
